...
ParticleSource::ParticleSource(float position[3], float color[4], std::shared_ptr<Physics::Buckets> &buckets, std::shared_ptr<Graphics::Base::Scene> *scene, float mass_per_time, float velocity[3])
:
...
ParticleSource::ParticleSource(float position[3], float color[4], std::shared_ptr<Physics::Buckets> &buckets, std::shared_ptr<Graphics::Base::Scene> *scene, float mass_per_time, float velocity[3], int max)
:
    d_mass_per_time(mass_per_time),
    d_scene(scene),
    d_buckets(buckets),
...
void ParticleSource::update(float t_step)
{
    ...
    for (int  i = 0; i != static_cast<int>(avg+d_rest); ++i)
    {
        ...
        (*d_scene)->particles->push_back(particle);
        Physics::Particle phys_particle(pos, d_buckets.get());
        (*d_scene)->particles->get_particles()->push_back(phys_particle);
        memcpy((*d_scene)->particles->get_particles()->back().velocity, d_velocity, sizeof(float)*3);
